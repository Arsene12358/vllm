...
"""Unit tests for SinkWindowSpec (Phase 1, Change A).
...
def test_sinkwindow_spec_set_membership():
    """Specs must be hashable so they can index spec_manager_map etc."""
    a = make_spec(start_size=64, sliding_window=512)
...
def test_uniform_type_all_same_sink_window():
    specs = {
        f"layer.{i}": make_spec(start_size=64, sliding_window=512)
        for i in range(4)
    }
...
def test_uniform_type_sink_window_mixed_with_full_attention_rejected():
    specs = {
        ...
        "layer.1": FullAttentionSpec(
            block_size=16, num_kv_heads=4, head_size=128,
            dtype=torch.bfloat16, sliding_window=None,
        ),
    }
    assert not UniformTypeKVCacheSpecs.is_uniform_type(specs)
...
class _MockVllmConfig:
    ...
    def __init__(self, max_model_len: int, max_num_batched_tokens: int) -> None:
        self.parallel_config = self._Parallel()
        self.model_config = self._Model(max_model_len)
        self.scheduler_config = self._Scheduler(max_num_batched_tokens)


def test_max_memory_usage_bytes_typical():
    """Sink+window typical: 64 + 512 + 2048 batch = 2624 tokens budget."""
    spec = make_spec(
        block_size=16, num_kv_heads=4, head_size=128,
        dtype=torch.bfloat16,
        start_size=64, sliding_window=512,
    )
    cfg = _MockVllmConfig(max_model_len=65536, max_num_batched_tokens=2048)
...
def test_max_memory_usage_bytes_capped_at_max_model_len():
    """When start+window+batch exceeds max_model_len, we cap."""
    spec = make_spec(start_size=64, sliding_window=8192)
    cfg = _MockVllmConfig(max_model_len=4096, max_num_batched_tokens=2048)
