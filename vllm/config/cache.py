...
class CacheConfig:
    ...
    def _validate_streaming_kv(self) -> "CacheConfig":
        ...
        if have_start:
            if self.streaming_kv_start_size is not None and self.streaming_kv_start_size <= 0:
                raise ValueError(
                    f"streaming_kv_start_size must be > 0, got "
                    f"{self.streaming_kv_start_size}"
                )
            if self.streaming_kv_recent_size is not None and self.streaming_kv_recent_size <= 0:
                raise ValueError(
